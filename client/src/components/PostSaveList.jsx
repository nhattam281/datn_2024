...
import { apiUnSavePost } from '../services/post';
import icons from '../utils/icons';
import PostDetailModal from './PostDetailModal';

const PostSaveList = ({ list }) => {
    ...
    const handleSearchPost = () => {
        try {
            dispatch(getAllPostSaved({ searchText }));
        } catch (error) {
            console.log(error);
            error &&
                Swal.fire({
    ...
    const handleUnSavePost = (row) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await apiUnSavePost(row.original.id);
                    console.log(res);
                    // res &&
                    Swal.fire({
                        title: 'Success...',
                ...
                return (
                    <div className='flex justify-center gap-2 text-white'>
                        <button
                            ...
                            onClick={() => handleUnSavePost(row)}
                        >
                            <icons.IoHeartDislike size={16} />
                        </button>
